...
const AddRecords = () => {
  ...
  const parseAllCountriesCSV = (csvText: string, nameToIso3: Map<string, string>): { iso3: string; count: number; raw: string }[] => {
    const lines = csvText.split("\n");
  ...
  const UploadCard: React.FC<{
    ...
    return (
      <div className="bg-white rounded-xl p-8 border border-gray-200 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
        ...
        <button
          onClick={onAddClick}
          className="w-full py-3.5 px-6 bg-white text-emerald-600 border-2 border-emerald-600 rounded-lg font-medium text-[15px] hover:bg-emerald-600 hover:text-white transition-all active:scale-[0.98]"
        >
          Add Record
        </button>
      </div>
  ...
  return (
    <div className="max-w-[1400px]">
      <div className="mb-10">
        <h1 className="text-[32px] font-semibold text-gray-900 mb-2">Add Records</h1>
